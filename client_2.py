"""
client_2.py — Connect to any deployed HiSLM server
=====================================================
Same as client.py but server address / port / client-name are
configurable via CLI args or environment variables, so this
connects to server_2.py (or any other HiSLM server) on any network.
...
Usage:
  # GUI (open browser, point to server_2):
  python client_2.py --server-ip 192.168.1.10 --port 8001

  # Headless / terminal mode:
  python client_2.py --server-ip 192.168.1.10 --port 8001 --cli --name my-node

  # Via environment variables:
  SERVER_IP=10.0.0.5 SERVER_PORT=8001 python client_2.py --cli

  # Custom port only (server on same machine):
  python client_2.py --server-ip 127.0.0.1 --port 9000 --cli
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("client_2.log", mode="a"),
    ],
)
log = logging.getLogger("CLIENT-2")
...
class WSCliClient:
    ...
    def _print_msg(msg: dict):
        ...
        text = msg.get("text", "")
        role_tag = "[SERVER]" if msg.get("role") == "server" else "[CLIENT]"
    ...
    def run_cli(self):
        ...
        print("\n─────────────────────────────────────────")
        print("  HiSLM Client 2 — Messenger  (CLI mode)")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="HiSLM client_2 — connects to any HiSLM server (server_2.py or server.py)"
    )
    p.add_argument("--server-ip", default=os.getenv("SERVER_IP", ""),
                   help="IP address of the server (required, or set $SERVER_IP)")
    p.add_argument("--port", type=int, default=int(os.getenv("SERVER_PORT", "8001")),
                   help="Server port (default: 8001 or $SERVER_PORT)")
    p.add_argument("--name", default=os.getenv("CLIENT_NAME", "client-node-2"),
                   help="Client identifier shown in chat (default: client-node-2 or $CLIENT_NAME)")
...
def main():
    ...
    if not args.server_ip:
        print("\n✗ --server-ip is required (or set $SERVER_IP)\n")
        print("  Example: python client_2.py --server-ip 192.168.1.10 --port 8001\n")
        sys.exit(1)

    base_url = f"http://{args.server_ip}:{args.port}"
    ws_url   = f"ws://{args.server_ip}:{args.port}/ws?client_id={args.name}"

    log.info("=" * 60)
    log.info("  HiSLM Node Messenger — Client 2")
    ...
    if not check_health(base_url):
        log.critical("[STARTUP] Server unreachable. Aborting.")
        print("\n✗ Cannot reach server.")
        print(f"  Make sure server_2.py is running on {args.server_ip}:{args.port}")
        ...
        try:
            ...
            for m in msgs:
                ...
                if mid not in seen_ids:
                    ...
                    sender = m.get("sender", "?")
                    role_tag = "[SERVER]" if m.get("role") == "server" else "[CLIENT]"
